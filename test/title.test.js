...
// 제목은 평문으로 쓰인다. 강조 기호가 그대로 보이면 안 된다.
test('제목의 마크다운 강조는 벗긴다', () => {
  const n = parseNote('a.md', '# LG생활건강 분석 (2026.02.22, **2026.06.03 갱신**)\n본문');
  assert.equal(n.title, 'LG생활건강 분석 (2026.02.22, 2026.06.03 갱신)');
});

test('강조 아닌 별표는 남긴다', () => {
  assert.equal(parseNote('a.md', '# 2 * 3 은 6\n본문').title, '2 * 3 은 6');
});
