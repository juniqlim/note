...
**재진입 트리거 3개(ⓐ Online GPM 개선 ⓑ Offline 비중 둔화 ⓒ OPM 반등) 점검 — 판정 보류**
- Profit Alert에 GPM·OPM·세그먼트 분해가 없어 ⓐⓑⓒ 모두 **확인 불가**. 8.28 중간보고서 대기.
- 다만 간접 신호는 엇갈린다: **(+)** NPM 6.1%로 1H2025(5.8%)·2H2025(3.5%) 대비 개선, 판관비 레버리지 발현 주장. **(−)** 매출 성장률 +49.3%→+23.6%로 반토막.
- 순이익률 개선이 ⓒ(OPM 반등)를 시사하나, 순이익은 금융비용·환율·세율 영향을 받으므로 **영업단 개선으로 단정할 수 없다.** 8.28에 OPM 실수치로 검증할 것.
