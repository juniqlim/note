...
> **핵심: AI 하드웨어 3사(하이닉스·NVIDIA·TSMC) 모두 같은 AI CapEx 사이클에 노출. 차이는 밸류에이션뿐.**
...
### 민감도 분석 A: 구조적 성장 (주당가치, 만원)

> 현재가: **84만원** | FCF: 80조(2026)→138조(2035)
...
> **모든 조합에서 현재가(84만) 대비 63~242% 상승 여력**. 가장 보수적 조합(WACC 12%, g 2%)에서도 137만원.

### 민감도 분석 B: 사이클 하락 (주당가치, 만원)

> 현재가: **84만원** | FCF: 80조(2026)→50→35→회복→83조(2035)
...
> 사이클 하락 + WACC 12% + g 2% = 83만원 ≈ **현재가 그 자체**. 시장은 이 시나리오를 100% 가격에 반영 중.
> WACC 13% 이상에서만 하방 리스크(-12%) 존재.

### 역산 DCF: 시장이 반영하는 할인율

구조적 성장(시나리오 A) FCF가 실현된다고 가정하고, 현재가 84만원에서 역산:

| 항목 | 값 |
|------|-----|
| 현재 시총 | ~638조원 |
| 시나리오 A FCF + TGR 3% 기준 내재 WACC | **~18%** |
| 참고: NVIDIA 내재 WACC (Bull Case) | ~8.8% |
| 참고: TSMC 내재 WACC (Base Case) | ~9.5% |

> **시장은 하이닉스에 WACC 18%를 적용 중**. NVIDIA(8.8%), TSMC(9.5%) 대비 **9pp+ 높은 리스크 프리미엄**.
> 이 차이의 본질 = 시클리컬 디스카운트. "이익이 지속되지 않을 것"이라는 시장의 확신.
...
|--|-----------|------|--------|
| 현재가 | ₩839,000 | $366 | $185 |
| fPER | **5~6x** | ~24x | ~24x (FY27E) |
| DCF Base Case | **+124%** (188만) | **-21%** ($289) | **-48%** ($97) |
| DCF Bear/사이클 | **±0%** (83만) | **-52%** ($177) | **-66%** ($62) |
| 내재 WACC | **~18%** | ~9.5% | ~8.8% |
| 시장 포지셔닝 | 비관 100% 반영 | 낙관 반영 | 극도의 낙관 반영 |
| 비대칭성 | **하방 제한, 상방 개방** | 양방향 리스크 | **하방 개방, 상방 제한** |

> **3사 모두 같은 AI CapEx 사이클에 노출**되어 있으나, 밸류에이션 격차가 극단적.
> 하이닉스만 유일하게 "나쁜 시나리오가 이미 가격에 반영된" 상태. NVIDIA/TSMC는 좋은 시나리오가 가격에 반영되어 있어 기대를 충족하지 못하면 하방 리스크가 큼.

---

## 13-1. DCF 업데이트 — CapEx 비례 상향 ver. (2026.02.24)

> 2월 하순 컨센서스 대폭 상향(OP 170조+) 및 주가 상승(84만→101만) 반영. CapEx를 매출 비례로 상향(26조→43조). **→ 13-2에서 CapEx 가정 재검토.**

### 변경된 전제

| 항목 | 02.09 (기존) | **02.24 (이 버전)** | 변동 |
|------|-------------|---------------------|------|
| **현재 주가** | ₩839,000 | **₩1,009,000** | +20% |
| **시가총액** | ~638조 | **~767조** | +20% |
| **2026 컨센서스 OP** | ~131조 (중앙값) | **~170조 (상단 그룹)** | +30% |
| **2026 FCF** | ~80조 | **~100조** | +25% |
| **FCF 산출** | NI 95조 + D&A 18조 - CapEx 26조 | **NI ~123조 + D&A ~20조 - CapEx ~43조** | CapEx 상향 |

> CapEx 상향 근거: NDR "매출대비 30% 중반 관리". 매출 230~244조 × 35% ≈ 80~85조 (현금 기준 ~43조 추정).

### 시나리오 A: 구조적 성장 (확률 75%, WACC 10%, TGR 3%)
...
- 내재가치: 768 + 874 = **1,641조** → **주당 ₩2,160,000 (+114%)**

### 시나리오 B: 사이클 (확률 25%, WACC 12%, TGR 2%)
...
| 2029 | +50% | 69 | 43.9 |
| 2030 | +16% | 80 | 45.4 |
| 2031~2035 | +5%/yr | 84→102 | 192.4 |

- 내재가치: 450 + 335 = **785조** → **주당 ₩1,030,000 (+2%)**

### 확률 가중: **₩1,870,000 (+86%)**, 안전마진 46%

> 이 버전의 문제: CapEx를 매출 비례로 43조까지 올렸으나, 설비투자 계획은 매출 추정과 무관하게 이미 확정. 13-2에서 수정.

---

## 13-2. DCF 업데이트 — CapEx 유지 ver. (2026.02.24)

> 13-1의 CapEx 가정 오류 수정. CapEx는 기존 26조 유지. 설비투자 계획은 이미 확정되어 있어 애널 매출 추정 상향과 무관.

### 변경된 전제

| 항목 | 02.09 (기존) | 13-1 (CapEx 상향) | **13-2 (CapEx 유지)** |
|------|-------------|-------------------|----------------------|
| **현재 주가** | ₩839,000 | ₩1,009,000 | **₩1,009,000** |
| **2026 OP** | ~131조 | ~170조 | **~170조** |
| **CapEx** | 26조 | **43조** | **26조** |
| **2026 FCF** | 80조 | 100조 | **117조** |

> 주가 +20%인데 FCF +46%인 이유: CapEx·D&A 고정비 차감 후 NI 증가분이 FCF에 레버리지로 작용.

### 시나리오 A: 구조적 성장 (에이전트 성공, 확률 75%)

**가정**: WACC 10%, TGR 3%, 2026 FCF 117조 기준
...
- 10년 FCF 현재가치 합계: **906조원**
- 잔존가치: 190 × 1.03 / 0.07 = 2,796조 → PV **1,078조**
- **내재가치 = 906 + 1,078 = 1,984조원**
- **주당 내재가치 = ₩2,610,000 (현재 대비 +159%)**

### 시나리오 B: 사이클 (2027~2028 다운사이클, 확률 25%)

**가정**: WACC 12%, TGR 2%, 2027~2028 FCF 급감 후 회복
...
- 10년 FCF 현재가치 합계: **525조원**
- 잔존가치: 1,214조 → PV **391조**
- **내재가치 = 525 + 391 = 916조원**
- **주당 내재가치 = ₩1,210,000 (현재 대비 +20%)**

### 확률 가중 내재가치

= 0.75 × ₩2,610,000 + 0.25 × ₩1,210,000 = **₩2,260,000**

| | 주당 가치 | 현재가 대비 |
|--|----------|-----------|
| 시나리오 A (구조적 성장) | ₩2,610,000 | **+159%** |
| 시나리오 B (사이클) | ₩1,210,000 | **+20%** |
| **확률 가중** | **₩2,260,000** | **+124%** |

### 핵심 인사이트 (02.09 대비 변화)

1. **주가 +20%인데 FCF +46% → 안전마진 오히려 확대** — 48% → 55%. CapEx 고정 + NI 레버리지 효과
2. **사이클 시나리오에서도 +20% 상승여력** — 기존 ±0%에서 개선. 시장이 사이클 100% 반영해도 여전히 싸다
3. **비대칭성 강화**: 구조적이면 +159%, 사이클이어도 +20%
4. **역산 WACC ~20%** — 기존 18%에서 상승. 시장이 요구하는 리스크 프리미엄이 오히려 높아진 것 (= 더 비관적으로 가격 반영)

### 민감도 분석 A: 구조적 성장 (주당가치, 만원)

> 현재가: **101만원** | FCF: 117조(2026)→190조(2035)
...
> **모든 조합에서 현재가(101만) 대비 89~294% 상승 여력**. 가장 보수적 조합(WACC 12%, g 2%)에서도 191만원.

### 민감도 분석 B: 사이클 하락 (주당가치, 만원)

> 현재가: **101만원** | FCF: 117조(2026)→76→53→회복→119조(2035)
...
> 사이클 하락 시에도 WACC 12% + g 2% = 121만원으로 **현재가 대비 +20%**. 모든 조합에서 현재가 이상.
> WACC 13% + g 1.5%에서도 107만원(+6%). 하방이 사실상 막혀 있다.

### DCF 이력 비교

| | 13 (02.09) | 13-1 (02.24, CapEx↑) | **13-2 (02.24, CapEx 유지)** |
|--|------------|----------------------|------------------------------|
| 현재가 | 84만 | 101만 | **101만** |
| CapEx | 26조 | 43조 | **26조** |
| 2026 FCF | 80조 | 100조 | **117조** |
| 시나리오 A | 188만 (+124%) | 216만 (+114%) | **261만 (+159%)** |
| 시나리오 B | 83만 (±0%) | 103만 (+2%) | **121만 (+20%)** |
| 확률 가중 | 162만 (+93%) | 187만 (+86%) | **226만 (+124%)** |
| 안전마진 | 48% | 46% | **55%** |
| 역산 WACC | ~18% | ~18% | **~20%** |

> **결론: 주가 +20% 올랐지만 FCF가 +46% 더 빠르게 상향. 안전마진이 48%→55%로 오히려 확대되었고, 사이클 시나리오에서도 +20% 상승여력이 생겼다.** 시장은 여전히 — 아니 오히려 더 강하게 — 사이클 하락만 반영 중.
...
## 부록 3. DCF 갱신 (2026-05-12, 신규 컨센서스 반영)

기존 섹션 13 "DCF 분석 (2026.02.09)"은 보존. 그 후 ① Q1 실적 확정(OP 37.6조, OPM 72%), ② 컨센서스 2026 OP 200~262조로 상향, ③ 주가 84만 → **190.3만원** (2.27배 상승), ④ 2025 CapEx 30.2조 (이전 21조 추정 대비 폭증) 반영해 재산정.

### 1. 입력 변수 갱신

| 항목 | 기존 (2/9) | 신규 (5/12) | 변경 |
|---|---|---|---|
| 현재가 | 84만원 | **190.3만원** | +127% |
| 시총 | ~$450B | **~$1.06T** | 2.3배 |
| 2025 실적 (확정) | (예상) | OP 47.2조, CapEx 30.2조 | DART |
| 2026E OP 컨센 | 101~199조 | **200~262조** (중앙 230) | 상향 |
| 2027E OP 컨센 | ~120~132조 | **300~394조** (중앙 340) | 대폭 상향 |

### 2. FCF 추정 (베이스 케이스)

세율 22%, CapEx·D&A 자체 추정:

| (조원) | OP | NOPAT | CapEx | D&A | FCF |
|---|---|---|---|---|---|
| 2026E | 230 | 179 | 40 | 18 | **157** |
| 2027E | 300 | 234 | 45 | 20 | **209** |
| 2028E | 250 | 195 | 40 | 22 | **177** |
| 2029E | 180 | 140 | 35 | 22 | **127** |
| 2030E | 150 | 117 | 30 | 22 | **109** (영구 베이스) |

가정 근거:
- 2028: M15X 가동 + 삼성 HBM4 캐파 본격화로 가격 소폭 하락. OP 25~30% 하락 가정
- 2029: 사이클 다운 진입, CXMT 범용 D램 증설 + 마이크론 HBM 캐치업 본격화
- 2030: 정상화 사이클. 영구 FCF로 채택
...
|---|---|---|---|
| 10% | 219만 (+15%) | 235만 (+24%) | **255만 (+34%)** |
| 11% | 195만 (+2%) | 207만 (+9%) | 222만 (+17%) |
| 12% | **165만 (-13%)** | 175만 (-8%) | 187만 (-2%) |
| 13% | 142만 (-25%) | 150만 (-21%) | 159만 (-16%) |

**시나리오 B (사이클 정점 후 급락, 2028 OP 180·2029 100·2030 80조)**

| WACC \ TGR | 2.0% | 2.5% | 3.0% |
|---|---|---|---|
| 11% | 132만 (-31%) | 138만 (-27%) | 145만 (-24%) |
| 12% | **115만 (-40%)** | 120만 (-37%) | 126만 (-34%) |

### 4. 역산 DCF: 현재가가 반영하는 가정

현재가 190만원 = **WACC 11% + TGR 2.5%** 베이스 가정에 거의 정확히 부합.

| 시나리오 | 내재 WACC |
|---|---|
| 시나리오 A FCF + TGR 2.5% | **~11%** |
| 2/9 시점 내재 WACC | ~18% |
| 참고: NVIDIA (Bull) | ~8.8% |
| 참고: TSMC (Base) | ~9.5% |

**핵심 변화**: 시장 내재 WACC가 **18% → 11%로 7%p 압축**. 한국 메모리 디스카운트의 절반 이상이 이미 가격에 반영됨. 남은 갭(11% vs 9% 글로벌 AI 평균)이 **추가 리레이팅 여력**.

### 5. 결론 변화

| 시점 | 결론 |
|---|---|
| 2026-02-09 | 모든 조합에서 +63~242% 상승 여력 (시장이 사이클 하락 100% 반영) |
| **2026-05-12** | **베이스 WACC 11%·TGR 2.5%에서 거의 적정. WACC 10%로 압축 시 +15~34%, WACC 12% 유지 시 -2~13%. 시나리오 B(급락)면 -27~40%** |

**투자 시사점**:
1. **저평가 단계는 끝남**. 2/9에 사도 됐다는 게 사후 검증된 셈
2. 추가 업사이드는 ① ADR 상장 → WACC 추가 1~2%p 압축 (목표가 220~250만), ② 2028~29 사이클 다운 시점 지연 확인 (FCF 추정 상향)
3. 다운사이드는 ① 사이클 조기 진입(2027 하반기) → 시나리오 B로 이동, ② CXMT 범용 D램 폭증
4. **현재가는 베이스 케이스에 정확히 부합 = 사이클 가정에 매우 민감해진 구간**

### 부록 3 가정 한계

- CapEx·D&A는 자체 추정 (회사 가이던스 미공시). 실제 2026 CapEx 40조 가정은 보수적일 수 있음 (daily-news 5/4 신규 팹 19조 + 기존 21조 = 40조 부합)
- 영구 베이스 FCF 109조는 정상화 사이클 가정. 메모리 구조적 수요(AI 추론·에이전트)가 굳어지면 영구 FCF 150조+ → 모든 시나리오 +15~25% 상향
- 한동희 시나리오(2027 OP 376조) 그대로 적용 시 베이스 목표가 **+30~50% 추가** (290~330만)
