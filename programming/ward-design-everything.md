...
번역 노트 및 핵심 요약
이 글은 익스트림 프로그래밍(XP)과 위키(Wiki)의 창시자인 워드 커닝햄이 복잡한 이론보다 실천과 유연함을 강조하며 쓴 글입니다.

세 개의 둔덕(Three Bumps): 너무 먼 미래를 예측하여 과도한 설계를 하지 말고(YAGNI), 바로 앞의 변화에 유연하게 대응하라는 의미입니다.

에지(Edges) = 다형성(Polymorphism): 억지로 코드를 비틀어(if/else 등) 흐름을 제어하려 하지 말고, 객체 지향의 핵심인 다형성에 위임하여 자연스럽게 로직이 흘러가게 하라는 은유입니다.

두려움 없음(No Fear): 복잡한 방법론에 압도되지 말고, 단순하게 시작하고 실행하라는 격려입니다.