...
| 2000 | 개발자 | 업무 로직 구현, 테스트, 리팩터링 | Spring, Rails, Git, AWS | 웹 애플리케이션 뼈대, 서버 구매 |
| 2010 | 소프트웨어 엔지니어 | 코드+테스트+배포+운영+모니터링, 서비스 소유 | Docker, Kubernetes, React, 클라우드 | 실행 환경 설치, 서버 관리 |
| 2020 | 소프트웨어 엔지니어 | 원격 협업, 직접 코딩. AI 자동완성은 일부만 보조로 씀 | Copilot, ChatGPT, 원격 협업 도구 | 없음. 2010년대와 같음 |
...
- 2010년대: 서비스 회사에서 프로덕트 매니저가 표준이 됨. 무엇을 만들지는 정하지만 어떻게는 안 정함. SI에는 분석가와 PM이 옛 의미로 남아 있다.

2026년 이후 AI가 "어떻게"를 가져가면 개발자는 "무엇을"로 올라가야 하는데, 그 자리에 이미 PM이 있다. 개발자가 PM이 되느냐, PM이 개발자가 되느냐, 새 직종이 생기느냐가 갈리는 중이다.

## 2020년대 전반: 원격과 AI 보조

- 팬데믹 (2020): 원격 근무가 기본이 됐다. 협업이 문서와 비동기 소통 중심으로 옮겨갔다.
- GitHub Copilot (2021), ChatGPT (2022): AI가 코드를 제안했지만, 쓰고 판단하는 건 사람이었다. 자동완성과 검색을 대체하는 보조 도구였고, 그마저 쓰는 사람이 일부였다. 보안을 이유로 도입을 막은 회사가 많았고, 대부분의 개발자는 보일러플레이트를 여전히 직접 쳤다.
